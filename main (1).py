...
# Token filtering
ALLOW_NON_ASCII_LABELS = False
MIN_ALPHA_CHARS = 2
MAX_ALPHA_CHARS = 20
PRINT_FULL_DEV_OUTPUTS_EACH_STEP = True
...
def normalize_candidate_token_text(s: str) -> str:
    return s.lstrip(" ")
...
def token_is_reasonable_label_piece(tokenizer, tid: int) -> bool:
    ...
    stripped = normalize_candidate_token_text(s)
    if not stripped:
        return False

    if not ALLOW_NON_ASCII_LABELS and not stripped.isascii():
        return False

    # Only allow alphabetic word-like tokens, optionally with a leading space.
    if not re.fullmatch(r" ?[A-Za-z]+", s):
        return False

    alpha_only = stripped
    if len(alpha_only) < MIN_ALPHA_CHARS or len(alpha_only) > MAX_ALPHA_CHARS:
        return False

    # Reject tokens that are weird capitalization patterns or common structural words.
    common_bad_words = {
        "the", "and", "or", "if", "then", "else", "when", "output",
        "only", "category", "name", "classify", "text", "instruction",
    }
    if alpha_only.lower() in common_bad_words:
        return False
...
def build_candidate_pool(
    ...
    candidate_ids = [tid for tid in searchable_ids if tid not in exclude_set]
...
def label_name_from_pred(pred: int, pos_text: str, neg_text: str) -> str:
    return pos_text if pred == 1 else neg_text



def print_dev_outputs(
    model,
    tokenizer,
    ...
    cache = prepare_search_cache(model, tokenizer, dev_data, pos_tids, neg_tids)
    pos_id = pos_tids[0]
    neg_id = neg_tids[0]
    pos_text = label_str(tokenizer, pos_tids)
    neg_text = label_str(tokenizer, neg_tids)

    clean_pos = cache.clean_log_probs[:, pos_id]
    clean_neg = cache.clean_log_probs[:, neg_id]
    inj_pos = cache.inj_log_probs[:, pos_id]
    inj_neg = cache.inj_log_probs[:, neg_id]

    clean_pred = (clean_pos >= clean_neg).long()
    inj_pred = (inj_pos >= inj_neg).long()

    print(f"[DEV OUTPUTS] {step_tag}")
    for i, (_, y) in enumerate(dev_data):
        gold_name = label_name_from_pred(y, pos_text, neg_text)
        clean_name = label_name_from_pred(int(clean_pred[i].item()), pos_text, neg_text)
        inj_name = label_name_from_pred(int(inj_pred[i].item()), pos_text, neg_text)
        clean_margin = float((clean_pos[i] - clean_neg[i]).item())
        inj_margin = float((inj_pos[i] - inj_neg[i]).item())
        print(
            f"  dev[{i:03d}] gold={gold_name!r} clean={clean_name!r} inj={inj_name!r} "
            f"clean_margin(pos-neg)={clean_margin:+.4f} inj_margin(pos-neg)={inj_margin:+.4f}"
        )
...
def gcg_search_single_restart(
    ...
    if PRINT_FULL_DEV_OUTPUTS_EACH_STEP:
        print_dev_outputs(model, tokenizer, dev_data, cur_pos, cur_neg, step_tag=f"restart={restart_id} init")
    print("=" * 120)
    ...
    for step in range(NUM_OUTER_STEPS):
        ...
        if PRINT_FULL_DEV_OUTPUTS_EACH_STEP:
            print_dev_outputs(model, tokenizer, dev_data, cur_pos, cur_neg, step_tag=f"restart={restart_id} step={step + 1}")
...
def main():
    ...
    preview_ids = searchable_ids[:50]
    preview_texts = [repr(token_piece_str(tokenizer, tid)) for tid in preview_ids]
    print(f"[INFO] First filtered candidate tokens: {preview_texts}")
